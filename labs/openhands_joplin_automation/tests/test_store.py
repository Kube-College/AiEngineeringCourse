...
from openhands_controller.domain.errors import VersionConflict
...
def test_workflow_survives_restart_and_stale_writer_is_rejected(tmp_path):
    h = Harness(tmp_path)
    assert h.store.create_workflow(("demo/joplin", 1), "r1", budget_limit=5_000_000)
...
def test_existing_controller_database_gains_iteration_accounting(tmp_path):
    ...
    older = files("openhands_controller.persistence").joinpath("schema.sql").read_text().replace(
